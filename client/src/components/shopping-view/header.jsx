        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Avatar className="bg-black cursor-pointer">
              <AvatarFallback>
                <CircleUserRound className="w-2/3 h-2/3"></CircleUserRound>
              </AvatarFallback>
            </Avatar>
          </DropdownMenuTrigger>
          <DropdownMenuContent className="w-56">
  ...
  function handleNavigate(getCurrentMenuItem) {
    ...
    const currentFilter =
      getCurrentMenuItem.id !== "products"
        ? {
            productCategory: getCurrentMenuItem.id,
          }
        : null;
    ...
    location.pathname.includes("/products") && currentFilter !== null
      ? setSearchParams(
          new URLSearchParams(`?productCategory=${getCurrentMenuItem.id}`)
        )
          ...
          <div className="ml-auto max-lg:mt-4">
            <ul className="flex items-center gap-4">
              ...
              <Link to="/search">
                <button
                  type="button"
                  class="rounded w-max h-10 text-[#333] text-sm tracking-wider font-medium outline-none border border-[#333] mx-4 cursor-pointer"
                >
                  <div class="flex items-center h-full">
                    <span class="flex-1 px-4">Tìm kiếm tại đây</span>
                    <div class="h-full w-12 flex justify-center items-center">
                      <Search></Search>
                    </div>
                  </div>
                </button>
        ...
        <ul className="lg:flex lg:flex-wrap lg:items-center lg:justify-center px-10 bg-[#212121] min-h-[46px] max-lg:space-y-4 max-lg:fixed max-lg:w-1/2 max-lg:min-w-[300px] max-lg:top-0 max-lg:left-0 max-lg:p-6 max-lg:h-full max-lg:shadow-md max-lg:overflow-auto z-50">
          ...
          {shoppingViewHeaderMenuItems.map((menuItem) => (
            <Label
              ...
              className="max-lg:border-b px-5 text-white text-[16px] font-medium block hover:bg-gray-700 py-5 uppercase font-roboto cursor-pointer"
              key={menuItem.id}
            >
              {menuItem.label}
            </Label>
